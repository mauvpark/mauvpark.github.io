...
지역 변수(메서드 내부에서 선언된 변수)를 사용할 때는 조심하세요. 만약 필드(클래스에 속한 변수)에 다시 값을 할당해야 한다면, 지역 변수에 할당하는 실수를 저지르지 않도록 하세요(지역 변수가 `final`이라면 이러한 실수를 예방할 수 있습니다). 그리고 만약 지역 변수가 스코프 안에 있을 때 필드를 변화시킨다면, 지역 변수는 오래된 값이 된다는 것에 주의하세요.
...
다트에는 함수 (메서드) 혹은 데이터 (객체 변수)가 될 수 있는 멤버라고 불리는 오브젝트를 가지고 있습니다.

#### [DON'T] Setter와 Getter에 불필요한 필드를 감싸지 마세요.

Java와 C#에서는 getter와 setter가 단순히 해당 필드로 포워딩 하는 역할만 할 뿐일지라도, 모든 필드를 getter와 setter에 숨기는 것이 흔합니다. 만약 이 요소들에 더 많은 작업이 필요하다면 다트에서는 호출 지점을 건드리지 않고도 그러한 작업들이 가능합니다. 이는 다트에서 하나의 getter method를 호출하는 것은 자바에서 한 필드에 접근한다를 의미를 가진 것과 다른 형태이기 때문입니다. 그리고 다트에서 한 프로퍼티에 접근하는 것은 C#에서 하나의 raw 필드에 접근하는 것과 바이너리 호환적이지 않습니다.
...
사람들은 `=>`를 사용해 코드를 _작성하는_ 것을 좋아하는 것처럼 보입니다만, 이 표현은 남용하기 매우 쉽고 이로 인해 코드가 _읽기_ 어려울 수 있습니다. 만약 선언부가 여러 라인으로 구성되거나 깊은 내포 표현(nested expression)을 포함하고 있다면 (가령 연속적인 표현 혹은 조건 연산자) 스스로나 당신의 코드를 읽는 모든 사람을 위해 블록 body와 어느 정도의 설명문을 곁들이는 것이 좋습니다.
